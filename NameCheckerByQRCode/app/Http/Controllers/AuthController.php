...
class AuthController extends Controller
{
    ...
    public function signin() {
        $url = $this->myauth->getSigninURL(route('loginpostback', ['_token' => csrf_token()]));
        return redirect($url);
    }
    ...
    public function signinPostback(Request $request) {
        ...
        $staff = $this->dc->find_student($username);
        dd($staff);
        ...
        //     if($user = User::where('student_id', $email)->first()) {
        //         Auth::login($user);
        //     } else {
        //         $newUser = new User();
        ...
        //     } else {
        //         $newUser = new User();
        //         $newUser->name = $student->full_name;
        //         $newUser->email = $email;
        //         $newUser->password = bcrypt(Str::random(16));
        //         $newUser->save();
        //         Auth::login($newUser);
