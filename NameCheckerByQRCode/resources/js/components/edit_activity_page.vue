    ...
    <div class="row">
        <div class="col">
                ...
                <div class="col d-flex flex-column flex-lg-row align-items-start align-items-lg-center justify-content-center justify-content-lg-start pt-3">
                    ...
                    <input class="form-check-input fs-5 chooseModeTypeDatasOS" type="radio" v-model="selectedOption" value="1" ref="haveList" >
                    <label class="form-check-label fs-5" for="radiosOSHaveData">
                        มีรายชื่อ
                    </label>
                    </div>
                    <div class="form-check col-12 col-lg-3">
                    <input class="form-check-input fs-5 chooseModeTypeDatasOS" type="radio" v-model="selectedOption" value="2" ref="no_haveList" >
            ...
            <div id="inputContainer">
                ...
                <div class="d-flex flex-column flex-lg-row justify-content-start justify-content-lg-center align-items-start align-items-lg-center mb-3">
                    <div class="col-12  col-lg-6 ">
                        ...
                        <input class="form-control datepicker getDate datepicker_costom_success" type="date" name="activity[date_add][date_input0][date]" :id="'date_input'+ item.id" placeholder="โปรดเลือกวันที่ก่อนจะใส่ข้อมูลช่องอื่น" autocomplete="off" :value="dateInputs[item.id] || item.date" @input="handleDateInput(item.id, $event)" :min="getMinDate()">
                        <div v-if="valueError && item.id === errorInputId && errorInputType === 'time_date'" v-html="errorHTML" class="text-danger text-center fs-6 fw-bold mt-2"></div>
                        {{ auto_validateDate(item.id,item.date) }}
                    </div>
                    <div class="col-12 col-lg-3 px-0 px-lg-3">
                        <label class="form-label fs-5">เวลาเริ่ม</label>
                        <input class="form-control get_IdInputTime revese-fake-disable" type="time" name="activity[date_add][date_input0][time][time_start]" :id="'timeStart_input'+item.id" :data-id="item.id" :value="selectedTime[item.id] ? selectedTime[item.id].start : item.time_start" @input="checkTime">
                    </div>
                    <div class="col-12 col-lg-3 px-0 px-lg-3">
                        <label class="form-label fs-5">เวลาจบ</label>
                        <input class="form-control get_IdInputTime revese-fake-disable" type="time" name="activity[date_add][date_input0][time][time_expried]" :id="'timeExpried_input'+item.id" :data-id="item.id" :value="selectedTime[item.id] ? selectedTime[item.id].expried : item.time_expried" @input="checkTime">
                ...
                <div class="d-flex flex-column mb-3">
                    <div class="d-flex flex-column flex-lg-row" id="div_SetCheckNamex0">
                        ...
                        <div class="form-check ms-4">
                            <input class="form-check-input get_IdsetCheckName" type="radio" value="1" :name="'activity'+item.id" :id="'setCheckName_day'+item.id" :data-id="item.id" @click="show_divCheckName">
                            <label class="form-check-label fs-5" for="setCheckNameI0">เช็คชื่อทั้งวัน</label>
                        </div>
                        <div class="form-check ms-4">
                            <input class="form-check-input get_IdsetCheckName" type="radio" value="2" :name="'activity'+item.id" :id="'setCheckName_round'+item.id" :data-id="item.id" @click="show_divCheckName">
                            <label class="form-check-label fs-5" for="setCheckNameII0">เช็คชื่อเป็นรอบ</label>
                        </div>
                        {{ auto_click_check(item.round_mode,item.id) }}
                    </div>
                    <div :id="'CheckName_allDay'+item.id" class="d-flex justify-content-center align-items-center div_CheckName p-5">
                        <h1 class="fs-3 fw-bold text-center">ระบบจะมีการเช็คชื่อตั้งแต่เวลาเริ่มจนเวลาจบ</h1>
                    </div>
                        <div :id="'CheckName_RoundDay'+item.id" class="d-flex flex-column div_CheckName p-0 p-lg-2 d-none">
                            <div class="d-flex flex-column-reverse flex-lg-row justify-content-start justify-content-lg-around align-itens-start align-items-lg-center w-100">
                                <div class="col-12 col-lg-7 d-flex flex-column flex-lg-row">
                                    ...
                                    <select class="form-select h-50 w-100 w-lg-75 me-0 me-lg-3 get_IdselectRound revese-fake-disable" :id="'selectSetRoundCheck'+item.id" :data-id="item.id" @change="showSelectRound">
                                ...
                                <div class="col-12 col-lg-5 d-flex flex-column ps-0 ps-lg-5">
                                    ...
                                    <select class="form-select w-100 w-lg-75 getId_selectTimeCheck revese-fake-disable" :id="'selectTimeCheck'+item.id" :data-id="item.id" @change="effect_inputduration">
                                        <option disabled="" hidden="">เลือกเวลา</option><option value="15">15 นาที</option>
                                        <option value="30">30 นาที</option><option value="45">45 นาที</option>
                                        <option value="60">60 นาที</option><option value="90">90 นาที</option>
                                    ...
                                    <div v-for="(round, index) in item.round" :key="index">
                                        ...
                                        <div class="d-flex flex-column flex-lg-row justify-content-center justify-content-lg-start align-items-start align-items-lg-center" id="div_row0">
                                            ...
                                            <div class="col-12 col-lg-6  d-flex my-2 ms-0 ms-lg-3 justify-content-center" :id="'divSpace'+item.id">
                                                <input class=" my-1 my-lg-4 bg-disable w-100 w-lg-75 border-0 fs-6 fs-lg-5 text-lg-center" disabled="" :id="'Boxinput'+item.id" :value="'จะเริ่มเช็คชื่อเมื่อเวลา : '+formatTime(round.rounde_checker_time_start)+' - '+formatTimesub(round.rounde_checker_time_expried)">
                                            </div>
                                        </div>
                                    </div>
                                </div>
